function [fluid_matrix, fluid_fracture] = initDeckADIFluidDP(deck, varargin)
%Initialize AD-solver fluid from ECLIPSE-style input deck

%Adapted for Dual Porosity Models
...
[reg_mat , reg_frac] = handleRegionsDP(deck, opt.G);
...
for k = 1:numel(fns)
    ...
    if doAssign(fn)
        ...
        try
            fluid_matrix = asgn(fluid_matrix, props.(fn), reg_mat);
...
fluid_fracture = struct();
for k = 1:numel(fns)
    fn = fns{k};
    if doAssign(fn)
        asgn = str2func(['assign',fn]);
        try
            fluid_fracture = asgn(fluid_fracture, props.(fn), reg_frac);
        catch ME
            warning(msgid('Assign:Failed'), ...
            'Could not assign property ''%s''. Encountered error: ''%s''',...
                fn, ME.message);
...
if isfield(fluid_matrix, 'bO') & isfield(fluid_matrix, 'bW') & isfield(fluid_matrix, 'bG')
    fluid_matrix.names={'WATER', 'OIL', 'GAS'};
elseif isfield(fluid_matrix, 'bO') & isfield(fluid_matrix, 'bW')
    fluid_matrix.names={'WATER', 'OIL'};
  
end 

if isfield(fluid_fracture, 'bO') & isfield(fluid_fracture, 'bW') & isfield(fluid_fracture, 'bG')
    fluid_fracture.names={'WATER', 'OIL', 'GAS'};
elseif isfield(fluid_fracture, 'bO') & isfield(fluid_fracture, 'bW')
    fluid_fracture.names={'WATER', 'OIL'};
