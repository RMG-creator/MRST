classdef GenericBlackOilSurfactantModel < GenericBlackOilModel
    properties
        surfactant
    end

    methods
        function model = GenericBlackOilSurfactantModel(G, rock, fluid, varargin)
            model = model@GenericBlackOilModel(G, rock, fluid, varargin{:});
            model.FlowPropertyFunctions = ...
                SurfactantFlowPropertyFunctions(model);
            ...
            model.Components{end + 1} = ImmiscibleComponent('surfactant', 'W');
        end
